...
const Profile =({navigation,route})=> {
    ...
    console.log('profile me before', state.loading)
    useEffect(()=>{
...
const handleModel=()=>{
   ...
   <TouchableOpacity onPress={()=>setModalOpen(true)} style={{alignItems: 'center',
     justifyContent: 'center',borderRadius:12, backgroundColor:'#758AF2',marginVertical:5,height:56,width:56}}> 
                ...
                <View style={{justifyContent:'center',alignItems:'center'}}>
                 {6>state.account.length > 0?state.account.map((list)=>{return  (<View Key={list._id} style={styles.items}  ><TouchableOpacity style={{justifyContent:'flex-start',flexDirection:'row'}} onPress={()=>{navigation.navigate('Account2',{list})}}>
                 <Image style={{height:72,width:72,borderRadius:12}}source={{uri:list.pic}} />
                 
                     <Text   style={{color:'#3C3B3B',fontSize:16,fontFamily:'Poppins-Bold',textAlign:'center',margin:Heightt*.30}}>{list.accountuser}</Text></TouchableOpacity>
